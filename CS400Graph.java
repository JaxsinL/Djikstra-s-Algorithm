...
public class CS400Graph<NodeType, EdgeType extends Number>
    implements GraphADT<NodeType, EdgeType> {

  /**
   * Vertex objects group a data field with an adjacency list of weighted directed edges
   ...
   * @return true if the data can be inserted as a new vertex, false if it is already in
   *         the graph
  ...
  /**
   * Remove a vertex from the graph. Also removes all edges adjacent to the vertex from
   * the graph (all edges that have the vertex as a source or a destination vertex).
   * 
   * @param data the data item stored in the vertex to remove
   * @return true if a vertex with *data* has been removed, false if it was not in the
   *         graph
   * @throws NullPointerException if data is null
   */
   ...
   * @return true if the edge could be inserted or its weight updated, false if the edge
   *         with the same weight was already in the graph
   * @throws IllegalArgumentException if either source or target or both are not in the
   *                                  graph, or if its weight is < 0
  ...
  public boolean insertEdge(NodeType source, NodeType target, EdgeType weight) {
    ...
    if (sourceVertex == null || targetVertex == null)
      throw new IllegalArgumentException(
          "Cannot add edge with vertices that do not exist");
   ...
   * @throws IllegalArgumentException if either sourceVertex or targetVertex or both are
   *                                  not in the graph
   * @throws NullPointerException     if either sourceVertex or targetVertex or both are
   *                                  null
  ...
  public EdgeType getWeight(NodeType source, NodeType target) {
    if (source == null || target == null)
      throw new NullPointerException(
          "Cannot contain weighted edge adjacent to null data");
    Vertex sourceVertex = vertices.get(source);
    Vertex targetVertex = vertices.get(target);
    if (sourceVertex == null || targetVertex == null)
      throw new IllegalArgumentException(
  ...
  /**
   * Path objects store a discovered path of vertices and the overal distance of cost of
   * the weighted directed edges along this path. Path objects can be copied and extended
  ...
  protected class Path implements Comparable<Path> {
    ...
    /**
     * Creates a new path containing a single vertex. Since this vertex is both the start
     * and end of the path, it's initial distance is zero.
     * 
     ...
     * without affecting the original path object (copyPath). The path is then extended by
     * the Edge object extendBy. Use the doubleValue() method of extendBy's weight field
     ...
     * @return -1 when this path has a smaller distance than the other, +1 when this path
     *         has a larger distance that the other, and the comparison of end vertex data
     *         in string form when these distances are tied
     */
  ...
  /**
   * Uses Dijkstra's shortest path algorithm to find and return the shortest path between
   * two vertices in this graph: start and end. This path contains an ordered list of the
   * data within each node on this path, and also the distance or cost of all edges that
   * are a part of this path.
   * 
   ...
   * @throws NoSuchElementException when no path from start to end can be found, including
   *                                when no vertex containing start or end can be found
   */
  protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    // If it does not include the start vertex, throw NoSuchElementException
    if (!vertices.containsKey(start)) {
      throw new NoSuchElementException("We can not find the start vertex from the path");
    }

    // If it does not include the end vertex, throw NoSuchElementException
    if (!vertices.containsKey(end)) {
      throw new NoSuchElementException("We can not find the end vertex from the path");
    }

    List<Vertex> coverVex = new LinkedList<Vertex>(); // Set the list of vertexes be
                                                      // covered

    Hashtable<Vertex, Path> shortest = new Hashtable<Vertex, Path>();// trace the shortest
                                                                     // distance

    // Hold the discovered paths (including total cost distance, and previous vertex data)
    // that have not yet been determined to be the shortest possible paths to their given
    // end vertex.
    PriorityQueue<Path> discoverPath = new PriorityQueue<Path>();
    discoverPath.add(new Path(vertices.get(start)));

    // This while loop is based on Florian's No.31 slide note
    while (discoverPath.isEmpty() == false) {
      Path temp = discoverPath.poll();

      if (!coverVex.contains(temp.end)) {
        shortest.put(temp.end, temp);
      }

      coverVex.add(temp.end); // Add the current visited vertex to the list which contains
                              // all visited vertex

      // Add every edge as a new path from the current vertex
      for (Edge extendBy : temp.end.edgesLeaving) {
        if (coverVex.contains(extendBy.target) == false) {
          discoverPath.add(new Path(temp, extendBy));
        }
      }
    }

    // Check if the shortest path is found
    if (shortest.containsKey(vertices.get(end))) {
      return shortest.get(vertices.get(end));

    } else {
      // throw NoSuchElementException if path is not found
  ...
  /**
   * Returns the shortest path between start and end. Uses Dijkstra's shortest path
   * algorithm to find the shortest path.
   * 
   * @param start the data item in the starting vertex for the path
   * @param end   the data item in the destination vertex for the path
   * @return list of data item in vertices in order on the shortest path between vertex
   *         with data item start and vertex with data item end, including both start and
   *         end
   * @throws NoSuchElementException when no path from start to end can be found including
   *                                when no vertex containing start or end can be found
   */
  ...
  /**
   * Returns the cost of the path (sum over edge weights) between start and end. Uses
   * Dijkstra's shortest path algorithm to find the shortest path.
   * 
   * @param start the data item in the starting vertex for the path
   * @param end   the data item in the end vertex for the path
   * @return the cost of the shortest path between vertex with data item start and vertex
